...
// define word type conversion function
...
//  txBuffer = (uint16_t*) calloc(ku8MaxBufferSize, sizeof(uint16_t));
  ...
  _RTUFrameGap = ceil((35.0*1000000)/_u32SerialBaudrate);
  if (_u32SerialBaudrate > 19200) {
    _RTUFrameGap = 1500;
  }
...
/**
Set a new slave id for the serial port.

@param u8MBSlave in standard increments (0..255)
*/
void ModbusMaster::setSlaveID(uint8_t u8MBSlave) {
  _u8MBSlave = u8MBSlave;
}

/**
Set a new baudrate for the serial port.
*/
void ModbusMaster::setBaudrate(uint32_t baudrate) {
  _u32SerialBaudrate = baudrate;
}
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  // Delay for starting a new transmission
  uint16_t elapsedMicros = micros() - _frameEnd;
  if (elapsedMicros < _RTUFrameGap) {
    delayMicroseconds(_RTUFrameGap - elapsedMicros);
  }
